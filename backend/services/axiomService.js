/**
 * Axiom Service - Enhanced with Multi-Controller Support
...
/**
 * Execute Axiom command over SSH using direct binary (default controller)
 * @param {string} command - Command name (e.g., 'ls', 'select', 'init')
 * @param {array} args - Command arguments
 * @returns {Promise} - Result with success, output, error
 */
...
/**
 * Execute command on specific controller (multi-controller support)
 * @param {object} controller - Controller object from database
 * @param {string} command - Full command to execute
 * @returns {Promise} - Result with success, output, exitCode
 */
async function executeCommandForController(controller, command) {
    const { Client } = require('ssh2');
    const conn = new Client();

    return new Promise((resolve, reject) => {
        conn.on('ready', () => {
            console.log(`[Axiom] Connected to controller: ${controller.name} (${controller.host})`);
            ...
            conn.exec(command, (err, stream) => {
                ...
                stream.on('close', (code) => {
                    ...
                    console.log(`[Axiom] Command completed with exit code: ${code}`);
        ...
        conn.on('error', (err) => {
            console.error(`[Axiom] SSH connection error:`, err);
            reject(err);
        });
...
/**
 * Execute Axiom binary on specific controller
 * @param {object} controller - Controller object
 * @param {string} commandName - Command name (e.g., 'ls', 'fleet')
 * @param {array} args - Arguments
 */
async function executeAxiomCommandForController(controller, commandName, args = []) {
    const binary = COMMAND_MAP[commandName];
    ...
    const binaryPath = `${AXIOM_CONFIG.binaryDir}/${binary}`;
    const argsString = args.join(' ');
    const fullCommand = `${binaryPath} ${argsString}`;

    return await executeCommandForController(controller, fullCommand);
}
...
module.exports = {
    ...
    executeCommandForController,
    executeAxiomCommandForController
};
