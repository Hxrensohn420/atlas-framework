...
# ============================================================================
# Atlas Framework - GCP Deployment Script
# ============================================================================

# Run this script ON the GCP instance after launching it.
# This script installs all dependencies and deploys Atlas Framework.

# USAGE:
# 1. Launch GCP instance (e2-medium, Ubuntu 22.04)
# 2. SSH into instance
# 3. Clone repo: git clone https://github.com/Hxrensohn420/atlas-framework.git
# 4. Run: cd atlas-framework && chmod +x deploy-gcp.sh && ./deploy-gcp.sh
...
# Get GCP instance public IP automatically
...
echo -e "${GREEN}╔════════════════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║ Atlas Framework - GCP Deployment Script            ║${NC}"
echo -e "${GREEN}╚════════════════════════════════════════════════════════╝${NC}"
echo ""
echo -e "${BLUE}📍 Detected GCP IP: ${GCP_IP}${NC}"
echo ""
...
NODE_ENV=production
API_PORT=5000
CDN_URL=http://${GCP_IP}:3000

# Security
JWT_SECRET=${JWT_SECRET}
JWT_EXPIRY=24h
ENCRYPTION_KEY=${ENCRYPTION_KEY}
CORS_ORIGIN=http://${GCP_IP}:3000
...
🌐 ACCESS URLS:
Dashboard:    http://${GCP_IP}:3000
Backend API:  http://${GCP_IP}:5000
Kong Gateway: http://${GCP_IP}:8000
Kong Admin:   http://${GCP_IP}:8001
...
echo ""
echo -e " 🌐 Dashboard:    ${BLUE}http://${GCP_IP}:3000${NC}"
echo -e " 🔌 Backend API:  ${BLUE}http://${GCP_IP}:5000${NC}"
echo -e " 🚪 Kong Gateway: ${BLUE}http://${GCP_IP}:8000${NC}"
echo -e " 📊 Kong Admin:   ${BLUE}http://${GCP_IP}:8001${NC}"
echo ""
...
echo -e " 3. ${BLUE}Verify Firewall${NC} allows ports: 3000, 5000, 8000"
echo ""
